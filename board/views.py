...
def new_item(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            issued_item = Item.objects.create(name = request.POST["name"], category_name = issued_category, amount = request.POST["amount"])
            issued_item.save()
            LogEntry.objects.log_action(
                        user_id=request.user.id,
                        content_type_id=ContentType.objects.get_for_model(Item).pk,
                        ...
                        object_repr=issued_item.name,
                        action_flag=ADDITION,
                        change_message=f"new item amount {request.POST['amount']} placed in {issued_category.name}")
        return redirect(request.META.get('HTTP_REFERER', '/'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            LogEntry.objects.log_action(
                        ...
                        object_repr=issued_item.name,
                        action_flag=CHANGE,
                        change_message=f"replaced to {new_category.name}")
        return redirect(request.META.get('HTTP_REFERER', '/'))